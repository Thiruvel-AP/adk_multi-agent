...
async def router_call(
    ...
    try:
        ...
        if not session_id or not user_id:
            logger.warning("User ID or session ID is missing, can't connect with the socket")
        ...
        # Log the error
        logger.error(f"Error at the router: {error}", exc_info=True)
...
async def transfer_session_id(
    ...
    try:
        ...
        if (
            not session_id
            or not user_id
            or not await SingletonSessionMemory.get_session(
                user_id=user_id,
                session_id=session_id,
            )
        ):
            ...
            try:
                ...
                logger.info(f"New session created - Session ID: {session_id}, User ID: {user_id}")
            except Exception as session_error:
                logger.error(f"Failed to create session: {session_error}", exc_info=True)
                raise HTTPException(status_code=500, detail=f"Failed to create session: {session_error}")
                

        return {"session_id": session_id, "user_id" : user_id}
    except Exception as e:
        logger.error(f"Error in transfer_session_id: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

if __name__ == "__main__":
    logger.info("Server started on http://0.0.0.0:8000")
    logger.info("WebSocket endpoint: http://0.0.0.0:8000/voice")
    logger.info("WebSocket endpoint: http://0.0.0.0:8000/start-session")
